    ...
    function renderAttemptBanner() {
      ...
      if (remainingAttempts > 0) {
        c.innerHTML = `<div class="alert alert-info text-center mb-0">
          You have <strong>${remainingAttempts}</strong> free attempt${remainingAttempts>1?'s':''} remaining for <strong>premium exams</strong>.
        </div>`;
      } 
  ...
  <div class="col-lg-4 col-md-6">
    <div class="card h-100 shadow-sm border-0 exam-card">
      <div class="position-relative">
        <img src="{{ asset('') }}${exam.examImg}"
             class="card-img-top exam-card-img rounded-top"
             alt="${exam.examName}">
        <!-- duration at top-left -->
        <div class="position-absolute top-0 start-0 m-2">
          <span class="badge bg-light text-dark">
            <i class="far fa-clock"></i> ${exam.totalTime || 'N/A'}m
          </span>
        </div>
      ...
      <div class="card-body d-flex flex-column p-3">
        ...
        <p class="card-text text-muted small mb-2">
          ${exam.examDescription?.substring(0,80) || 'No description'}${exam.examDescription?.length > 80 ? '…' : ''}
        </p>
        <div class="mb-3">
          <span class="badge bg-secondary">${exam.associated_course || 'N/A'}</span>
        </div>
        <div class="mt-auto d-flex border-top pt-2 gx-2">
          <!-- Attempts -->
          ...
          <div class="flex-fill text-center">
            ${
              isStudentLoggedIn() && exam.user_attempt_count >= exam.total_attempts
...
document
  ...
  .addEventListener('submit', async e => {
    ...
    // store token & student id
    storeStudentToken(res.student_token, res.tokenable_type, res.student_id);
    loginModal.hide();

    // refresh global attempt counts
    await fetchRemainingAttempts();

    // now check per-exam attempts before starting
    if (pendingExam) {
      const { id, duration } = pendingExam;
      pendingExam = null;

      // call your has-attempts API
      const hasResp = await fetch(`/api/exam/${id}/has-attempts`, {
        headers: {
          'Accept':'application/json',
          'Authorization': `Bearer ${sessionStorage.getItem(tokenKey)}`
        }
      ...
      if (hasResp.ok && hasJson.has_attempts) {
        // still allowed → start the exam
        startExam(id, duration);
      } else {
        // no more per-exam attempts → show modal then reload
        await Swal.fire({
          icon: 'warning',
          title: 'No Attempts Left',
          text: 'You have already used up all attempts for this exam.',
